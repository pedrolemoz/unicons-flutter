...
// Generated in 2024-08-26T20:39:58.301473

import 'dart:math' as math;

import 'package:flutter/material.dart';

class ForwadedCallIcon extends StatelessWidget {
  final Color? color;

  const ForwadedCallIcon({
    super.key,
    this.color,
  });
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        return CustomPaint(
          size: size,
          painter: ForwadedCallPainter(color: color ?? colorScheme.primary),
        );
      },
    );
  }
...
class ForwadedCallPainter extends CustomPainter {
  final Color color;
  ...
  void paint(Canvas canvas, Size size) {
    final scaleX = size.width / 20.136949012883637;
    final scaleY = size.height / 19.892550255967844;
    final scale = math.min(scaleX, scaleY);

    final translationX = (size.width - 20.136949012883637 * scale) / 2 - 1.890774581827165 * scale;
    ...
    path.cubicTo(
      17.342276405289205 * scale + translationX,
      7.750037454310924 * scale + translationY,
      17.3422764052892 * scale + translationX,
    ...
    path.cubicTo(
      18.120037454310925 * scale + translationX,
      9.157723594710799 * scale + translationY,
      18.749962545689076 * scale + translationX,
      9.157723594710799 * scale + translationY,
    ...
    path.cubicTo(
      ...
      5.879962545689077 * scale + translationY,
      22.0277235947108 * scale + translationX,
      5.250037454310922 * scale + translationY,
      21.64 * scale + translationX,
      4.8599999999999985 * scale + translationY,
    );
    ...
    path.cubicTo(
      18.7432397225443 * scale + translationX,
      2.0202232040712964 * scale + translationY,
      18.151805828454297 * scale + translationX,
      2.0430677303189664 * scale + translationY,
      17.782436779386632 * scale + translationX,
      2.4124367793866295 * scale + translationY,
    );

    path.cubicTo(
      17.413067730318968 * scale + translationX,
      2.7818058284542926 * scale + translationY,
      17.390223204071297 * scale + translationX,
      3.373239722544299 * scale + translationY,
      17.73 * scale + translationX,
      3.769999999999999 * scale + translationY,
    );

    path.lineTo(
      18.52 * scale + translationX,
      4.56 * scale + translationY,
    );
    ...
    path.cubicTo(
      14.387715250169206 * scale + translationX,
      4.56 * scale + translationY,
      13.94 * scale + translationX,
      5.007715250169206 * scale + translationY,
      13.94 * scale + translationX,
      5.56 * scale + translationY,
    );

    path.cubicTo(
      13.94 * scale + translationX,
      6.112284749830793 * scale + translationY,
      14.387715250169206 * scale + translationX,
      6.56 * scale + translationY,
      14.94 * scale + translationX,
      6.56 * scale + translationY,
    );
