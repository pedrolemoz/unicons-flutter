...
// Generated in 2024-08-26T20:39:59.766682

import 'dart:math' as math;

import 'package:flutter/material.dart';

class PhonePauseIcon extends StatelessWidget {
  final Color? color;

  const PhonePauseIcon({
    super.key,
    this.color,
  });
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        return CustomPaint(
          size: size,
          painter: PhonePausePainter(color: color ?? colorScheme.primary),
        );
      },
    );
  }
...
class PhonePausePainter extends CustomPainter {
  final Color color;

  const PhonePausePainter({
    super.repaint,
    required this.color,
  });
  ...
  void paint(Canvas canvas, Size size) {
    final scaleX = size.width / 20.023204943675967;
    final scaleY = size.height / 19.892550255967844;
    final scale = math.min(scaleX, scaleY);

    final translationX = (size.width - 20.023204943675967 * scale) / 2 - 1.890774581827165 * scale;
    ...
    path.cubicTo(
      19.378057237330687 * scale + translationX,
      19.952207850306177 * scale + translationY,
      19.077003823074634 * scale + translationX,
      20.043992427823266 * scale + translationY,
