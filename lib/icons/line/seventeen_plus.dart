...
// Generated in 2024-08-26T20:39:55.402328

import 'dart:math' as math;

import 'package:flutter/material.dart';

class SeventeenPlusIcon extends StatelessWidget {
  final Color? color;

  const SeventeenPlusIcon({
    super.key,
    this.color,
  });
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        return CustomPaint(
          size: size,
          painter: SeventeenPlusPainter(color: color ?? colorScheme.primary),
        );
      },
    );
  }
...
class SeventeenPlusPainter extends CustomPainter {
  final Color color;
  ...
  void paint(Canvas canvas, Size size) {
    ...
    path.lineTo(
      15.780000000000001 * scale + translationX,
      8.0 * scale + translationY,
    );
    ...
    path.cubicTo(
      13.941298066569379 * scale + translationX,
      17.065749563634466 * scale + translationY,
      13.993446216218967 * scale + translationX,
      17.34394536238076 * scale + translationY,
      14.144411059360472 * scale + translationX,
      17.570392627093018 * scale + translationY,
    );

    path.cubicTo(
      14.295375902501977 * scale + translationX,
      17.796839891805277 * scale + translationY,
      14.532115043787178 * scale + translationX,
      17.95197502946116 * scale + translationY,
    ...
    path.cubicTo(
      15.482760763749651 * scale + translationX,
      18.009735705397528 * scale + translationY,
      15.903526374198885 * scale + translationX,
      17.67312321703814 * scale + translationY,
    ...
    path.cubicTo(
      18.057962372447133 * scale + translationX,
      6.905841745498037 * scale + translationY,
      17.980883636116364 * scale + translationX,
      6.601197216190715 * scale + translationY,
      17.79 * scale + translationX,
      6.369999999999999 * scale + translationY,
    );

    path.cubicTo(
      17.59717715332323 * scale + translationX,
      6.132253462596547 * scale + translationY,
      17.30608399036397 * scale + translationX,
      5.995918690071323 * scale + translationY,
