...
// Generated in 2024-08-26T20:39:58.114389

import 'dart:math' as math;

import 'package:flutter/material.dart';

class FilesLandscapesAltIcon extends StatelessWidget {
  final Color? color;

  const FilesLandscapesAltIcon({
    super.key,
    this.color,
  });
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        return CustomPaint(
          size: size,
          painter: FilesLandscapesAltPainter(color: color ?? colorScheme.primary),
        );
      },
    );
  }
...
class FilesLandscapesAltPainter extends CustomPainter {
  final Color color;

  const FilesLandscapesAltPainter({
    super.repaint,
    required this.color,
  });
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final scaleY = size.height / 18.000000398938848;
    final scale = math.min(scaleX, scaleY);

    final translationX = (size.width - 22.000000761078784 * scale) / 2 - 0.9999999737527399 * scale;
    final translationY = (size.height - 18.000000398938848 * scale) / 2 - 2.999999955133959 * scale;
    ...
    path.cubicTo(
      16.278414556309134 * scale + translationX,
      3.0517413091534458 * scale + translationY,
      16.166223105545754 * scale + translationX,
      3.0143441588989854 * scale + translationY,
      16.04999975996668 * scale + translationX,
      2.999999955133959 * scale + translationY,
    );
