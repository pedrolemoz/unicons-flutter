...
// Generated in 2024-08-26T20:39:57.744728

import 'dart:math' as math;

import 'package:flutter/material.dart';

class ExclamationCircleIcon extends StatelessWidget {
  final Color? color;
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        return CustomPaint(
          size: size,
          painter: ExclamationCirclePainter(color: color ?? colorScheme.primary),
        );
      },
    );
  }
...
class ExclamationCirclePainter extends CustomPainter {
  ...
  const ExclamationCirclePainter({
    super.repaint,
    required this.color,
  });
  ...
  void paint(Canvas canvas, Size size) {
    ...
    path.cubicTo(
      ...
      14.000000218903361 * scale + translationY,
      10.750000194011351 * scale + translationX,
      14.559644282385968 * scale + translationY,
      10.750000197440386 * scale + translationX,
      15.250000214825072 * scale + translationY,
    );

    path.cubicTo(
      10.75000020086942 * scale + translationX,
      15.940356147264175 * scale + translationY,
      11.309644258792463 * scale + translationX,
      16.50000020518722 * scale + translationY,
      12.000000191231567 * scale + translationX,
      16.500000208616257 * scale + translationY,
    );

    path.cubicTo(
      12.690356123670671 * scale + translationX,
      16.500000212045293 * scale + translationY,
      13.250000187153281 * scale + translationX,
      15.940356159681812 * scale + translationY,
      13.250000197440386 * scale + translationX,
      15.250000227242708 * scale + translationY,
    );

    path.cubicTo(
      13.25000020772749 * scale + translationX,
      14.559644275406054 * scale + translationY,
      12.690356130650589 * scale + translationX,
      14.000000198329152 * scale + translationY,
