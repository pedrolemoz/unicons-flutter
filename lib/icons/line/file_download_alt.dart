...
// Generated in 2024-08-26T20:39:57.939336

import 'dart:math' as math;

import 'package:flutter/material.dart';

class FileDownloadAltIcon extends StatelessWidget {
  final Color? color;

  const FileDownloadAltIcon({
    super.key,
    this.color,
  });
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        return CustomPaint(
          size: size,
          painter: FileDownloadAltPainter(color: color ?? colorScheme.primary),
        );
      },
    );
  }
...
class FileDownloadAltPainter extends CustomPainter {
  final Color color;

  const FileDownloadAltPainter({
    super.repaint,
    required this.color,
  });
  ...
  void paint(Canvas canvas, Size size) {
    final scaleX = size.width / 18.005798324341672;
    final scaleY = size.height / 20.02697657186813;
    final scale = math.min(scaleX, scaleY);

    final translationX = (size.width - 18.005798324341672 * scale) / 2 - 3.0 * scale;
    final translationY = (size.height - 20.02697657186813 * scale) / 2 - 2.0 * scale;
    ...
    path.cubicTo(
      17.385103709694295 * scale + translationX,
      21.801040629317033 * scale + translationY,
      17.497248682277572 * scale + translationX,
    ...
    path.cubicTo(
      17.86203970705545 * scale + translationX,
      22.026976571868126 * scale + translationY,
      18.137959944143237 * scale + translationX,
      22.02697657186813 * scale + translationY,
      18.379999821917554 * scale + translationX,
      21.919999787618764 * scale + translationY,
    );

    path.cubicTo(
      18.502751317722424 * scale + translationX,
      21.87240561187003 * scale + translationY,
      18.6148962903057 * scale + translationX,
      21.801040629317033 * scale + translationY,
